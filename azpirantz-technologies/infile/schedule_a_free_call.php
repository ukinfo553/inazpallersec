<script>
  function check_email(str) {
    event.preventDefault();
    var fullname = document.getElementById("fullname").value;
    var email = document.getElementById("email").value;
    var mobile = document.getElementById("mobile").value;
    var company = document.getElementById("company").value;
    var message = document.getElementById("message").value;
    var pagename = document.getElementById("pagename").value;

    let isValid = true;

    if (fullname === "" || /\d/.test(fullname)) {
      document.getElementById('fullname').style.border = "2px solid #E50914";
      isValid = false;
      return false;
    }
    else {
      document.getElementById('fullname').style.borderColor = "green";
    }

    if (email === "" || !email.includes("@")) {
      document.getElementById('email').style.border = "2px solid #E50914";
      isValid = false;
      return false;
    }
    else {
      document.getElementById('email').style.borderColor = "green";
    }

    if (mobile === "" || mobile.length < 6) {
      document.getElementById('mobile').style.border = "2px solid #E50914";
      isValid = false;
      return false;
    }
    else {
      document.getElementById('mobile').style.borderColor = "green";
    }


    if (isValid) {

      $.ajax({
        type: 'POST',
        url: 'schedule_mail.php',
        data: {
          contactFrmSubmit: 1,
          email: email,
          fullname: fullname,
          mobile: mobile,
          company: company,
          pagename: pagename,
          message: message

        },
        beforeSend: function () {
          $('.submitBtn').attr("disabled", "disabled");

          //$('.modal-body').css('opacity', '.5');
        },
        success: function (msg) {
          if (msg == 'ok') {
            //$('#inputName').val('');
            //$('#inputEmail').val('');
            //$('#inputMessage').val('');
            //$('.statusMsg').html('<span style="color:green;">Thanks for contacting us, we\'ll get back to you soon.</span>');
          } else {
            //$('.statusMsg').html('<span style="color:red;">Some problem occurred, please try again.</span>');
            $('.statusMsg').html('<span style="color:var(--green);font-family:var(--nunito);font-szie:14px;text-align:center;">Thanks for contacting us, we\'ll get back to you soon.</span>');
            //window.location.href = "https://www.azpirantz.com/registration/reg2/calendar_list_advance.php";
          }
          $('.submitBtn').removeAttr("disabled");
          //$('.modal-body').css('opacity', '');
        }
      });



    }
  }
</script>

<div class="col-12 schedule-free-call mt-5">
  <div class="item border-radius-10 bg-grey">
    <!--form start-->
    <form role="form" enctype="multipart/form-data" method="post" class="d-flex flex-column gap-3 w-100" id="myForm">
      <div class="fs-28 f-nunito text-dark text-center fw-bold">
        Schedule A Free Call
      </div>
      <div class="mail-status" id="mail-status" style=" color:#FFFFFF;"></div>
      <!--form input start -->
      <div class="d-flex gap-3 flex-column flex-lg-row">
        <div class="d-flex gap-3 flex-column">
          <div class="d-flex flex-sm-row flex-column gap-3">
            <div><input type="text" placeholder="full name*" name="fullname" id="fullname"></div>
            <div><input type="number" placeholder="Mobile*" name="mobile" id="mobile"></div>
          </div>
          <div class="d-flex flex-sm-row flex-column gap-3">
            <div><input type="email" placeholder="email*" name="email" id="email"></div>
            <div><input type="text" placeholder="your company" name="company" id="company"></div>
          </div>
        </div>
        <div class="d-flex flex-wrap flex-md-nowrap gap-3">
          <textarea name="message" id="message" placeholder="message"></textarea>
          <input type="hidden" name="pagename" id="pagename" value="<?php echo $currentpage; ?>">
        </div>
      </div>
      <!--form input end -->
      <button type="submit" class="button m-auto submitBtn" value="Submit" onClick="check_email(this.value)"
        name="update_opps">Send Message</button>
      <div class="statusMsg"></div>
    </form>
    <!--form end-->
  </div>
</div>