...
async function fetchCalendarData() {
  try {
    const response = await fetch("https://azpirantz.com/registration/api/calander_list_bypostid.php?webpostid=30764");
...
function populateTable(data) {
  ...
  const seatsLeftform = document.querySelector(".seats-left");
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // Check if max_registration equals totalreg and if seats are available
    const seatsLeft = course.max_registration - course.totalreg;
    if (seatsLeft > 0) {
      seatsLeftform.innerText = (`Next Batch: ${course.bsd_nf_short} ${seatsLeft} Seat Left`);
