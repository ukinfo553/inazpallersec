...
async function fetchCalendarData() {
  try {
    const response = await fetch("https://azpirantz.com/registration/api/calander_list_bypostid.php?webpostid=39518"); //39518
    ...
    seatsLeftform.forEach((seatsLeftform) => {
      if (seatsLeft <= 3) {
        seatsLeftform.innerHTML = `
              ...
              <span style="padding-right: 8px;">Seat Left</span>
            `;
      } else {
        seatsLeftform.innerHTML = `
              <span style="color: var(--green);">Next Batch: </span>
