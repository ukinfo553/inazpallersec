...
let rows = document.querySelectorAll('.table_training tr.row-2');
let trainingCalendarRow = document.querySelector('.training-calendar-row');
rows.forEach((row) => {
    let trainingModeCell = row.querySelector('td[data-label="Training Mode  "]');
    if (trainingModeCell && trainingModeCell.textContent.trim() === 'Classroom Hyderabad') {
        row.style.display = '';
    } else {
      trainingCalendarRow.style.display = 'none';
    }
});

let online_training = document.querySelectorAll('.online_training .table_training tr.row-2');
online_training.forEach((row) => {
    let trainingModeCell = row.querySelector('td[data-label="Training Mode  "]');
    if (trainingModeCell && trainingModeCell.textContent.trim() === 'Classroom Hyderabad') {
      trainingCalendarRow.style.display = 'none';
    } else {
