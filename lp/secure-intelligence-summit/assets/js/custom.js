...
const section = document.querySelector('.content-sec');
const blurBg = document.getElementById('blur-bg');

function handleScroll() {
    const sectionRect = section.getBoundingClientRect();
    const viewportHeight = window.innerHeight;
    const sectionTopInView = sectionRect.top <= 0;
    const sectionBottomPassed = sectionRect.bottom <= viewportHeight;
  
    if (sectionTopInView && !sectionBottomPassed) {
        blurBg.classList.add('is-fixed');
    } else {
        blurBg.classList.remove('is-fixed');
    }
}
